...
files = list.files(path = "wetransfer-306ba4/", pattern = "*.csv")
...
PP_final = list.files('wetransfer-c9d9f3', pattern = "*.csv", full.names = TRUE)
...
pp_file_list = list.files('wetransfer-c9d9f3', pattern = "*.csv", full.names = TRUE)
...
```{r}
setwd("~/Downloads/wetransfer-88619a")
data <- read.csv("processedData.csv")
...
summary(model)
#Adjusted R-squared:  0.07196 and no significant p-values
#Heart rate
model_h <- lm(RR_h ~ Condition * pairs, data = final_new_surro)
summary(model_h)
#Adjusted R-squared:  0.03549  and p-values are far far far away from being significant
...
summary(model_d_h)
#Adjusted R-squared: 0.04502, p-value = 0.989
...
summary(model_L_r)
#Adjusted R-squared: 0.4144, p-value = 0.472215
#Heart rate
model_L <- lm(L_h ~ Condition * pairs, data = final_new_surro)
summary(model_L)
#Adjusted R-squared: 0.0014113, p-value = 0.749
